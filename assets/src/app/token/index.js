...
.controller( 'TokenCtrl', ['$scope', 'titleService', function TokenController( $scope, titleService ) {
    ...
    $scope.viewContractInstance = $scope.tokenContract.at($scope.tokenAddress);

	//$scope.totalEth = $scope.ethWeb3.tokenContractInstance.getTotalEthRecieved();
	//$scope.totalToken = $scope.ethWeb3.tokenContractInstance.getTotalTokenSupply();
	//$scope.tokenPrice = $scope.ethWeb3.tokenContractInstance.getTokenPrice();
